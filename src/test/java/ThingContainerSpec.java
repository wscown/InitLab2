...
public class ThingContainerSpec {
    ...
    public void testAdd() {
        ...
        for (int j = 0; j < testValue1; j++) {
            assertEquals("Should return an enum type of RED within entry" + j + "testContainer1's array of ColorfulThings", Colors.RED, testContainer1.getColorfulThings()[j].getColor());
        }
        for (int j = 0; j < testValue2; j++) {
            assertEquals("Should return an enum type of RED within entry" + j + "testContainer2's array of ColorfulThings", Colors.RED, testContainer2.getColorfulThings()[j].getColor());
        }
    ...
    public void testPop() {
        ...
        assertEquals("Should have returned the last entered color object in the array which is purple", Colors.PURPLE, testContainer5.pop().getColor());
    ...
    public void testRemove(){
        ...
        assertEquals("Should return the first object in the array that has enum type BLUE", new ColorfulThing(Colors.BLUE).getColor(), testContainer3.remove(Colors.BLUE).getColor());
        ...
        assertEquals("Should return the first object in the array that matches the input Object", new ColorfulThing(Colors.YELLOW).getColor(), testContainer3.remove(new ColorfulThing(Colors.YELLOW)).getColor());
        assertEquals("Should return null as searching for enum type not contained in the array", null, testContainer3.remove(Colors.PURPLE));
